...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      if (widget.username != null) {
        user = await ApiService.getUserByUsername(widget.username!);
      } else {
  ...
  Future<void> _loadUserPosts(int userId) async {
    ...
    try {
      ...
      setState(() { _posts = posts; _isLoadingPosts = false; });
    } catch (e) {
  ...
  Future<void> _pickAndUploadCoverPhoto() async {
    ...
    try {
      ...
      final updated = await ApiService.uploadCoverPhoto(File(picked.path));
      setState(() => _user = updated);
        ...
        actions: [
          if (_isOwnProfile)
            PopupMenuButton<String>(
              ...
              onSelected: (val) { if (val == 'password') _openChangePassword(); },
              itemBuilder: (_) => [
                const PopupMenuItem(value: 'password', child: Row(children: [Icon(Icons.lock_outline, size: 18), SizedBox(width: 8), Text('Change Password')])),
              ],
            ),
        ],
      ),
        ...
        child: CustomScrollView(
          ...
          slivers: [
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: user.coverPhotoUrl != null
                      ...
                      errorBuilder: (ctx, e, s) => const Center(child: Icon(Icons.broken_image, size: 40, color: Colors.white)))
                      : Center(child: Icon(_isOwnProfile ? Icons.add_photo_alternate : Icons.photo, size: 40, color: Colors.white)),
                ),
              ),
            ),
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        // Avatar
                        GestureDetector(
                          onTap: _isOwnProfile ? _pickAndUploadProfilePhoto : null,
                          child: Stack(
                            children: [
                              CircleAvatar(
                                radius: 44,
                        ...
                        Expanded(
                          child: Row(
                            ...
                            children: [
                              ...
                              _StatTile(label: 'Followers', count: user.followersCount),
                              _StatTile(label: 'Following', count: user.followingCount),
                            ],
                          ),
                        ),
                      ],
                    ...
                    if (user.username != null)
                      Text('@${user.username}', style: const TextStyle(color: AppColors.textGrey, fontSize: 13)),
                    if (user.role != null)
                      Text(user.role!, style: const TextStyle(color: AppColors.textGrey, fontSize: 13)),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: ElevatedButton.icon(
                              ...
                              icon: _isFollowLoading
                                  ...
                                  : Icon(_isFollowing ? Icons.person_remove_outlined : Icons.person_add_outlined, size: 16),
                              label: Text(_isFollowing ? 'Following' : 'Follow', maxLines: 1, overflow: TextOverflow.ellipsis),
                              style: ElevatedButton.styleFrom(
                                backgroundColor: _isFollowing ? Colors.grey[300] : AppColors.primaryDark,
                          ...
                          Expanded(
                            child: OutlinedButton.icon(
                              onPressed: () async {
                                try {
                                  final conversationId = await ApiService.startConversation(user.id);
                                  if (mounted) {
            ...
            if (_isLoadingPosts)
              ...
              const SliverToBoxAdapter(child: SizedBox(height: 200, child: Center(child: Text('No posts yet', style: TextStyle(color: Colors.grey)))))
            else
              SliverGrid(
                delegate: SliverChildBuilderDelegate(
                    ...
                    final post = _posts[i];
                    return Container(
                      decoration: BoxDecoration(color: Colors.grey[200], border: Border.all(color: Colors.white, width: 1)),
                      child: post.imageUrl != null
                          ? Image.network(post.imageUrl!, fit: BoxFit.cover, errorBuilder: (ctx, e, s) => const Icon(Icons.broken_image))
                          : Container(
                        color: AppColors.primaryLight.withOpacity(0.3),
                        child: const Icon(Icons.text_snippet_outlined, color: AppColors.primaryDark),
                      ),
                    );
                  },
